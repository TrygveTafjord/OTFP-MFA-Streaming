...
class MFA(nn.Module):
    ...
    def stepwise_em_update(self, X, log_resp):
        ...
        for k in range(self.K):
            ...
            psi_update = (self.S_xx[k] - (mu_k_new * self.S1[k]) - diag_cross) / (self.S0[k] + 1e-10)
    ...
    def add_component(self, new_mu, new_Lambda, new_log_psi, new_S0, new_S1, new_S_xx, new_S_z, new_S_xz, new_S_zz):
        """
        Dynamically adds a new component to the Mixture Model and integrates its sufficient statistics.
        """
        with torch.no_grad():
            # 1. Handle Tensor Shape Matching (Padding Lambda and Latent Statistics)
            q_new = new_Lambda.shape[2]
            
            if q_new < self.q:
                pad_size = self.q - q_new
                # Pad the new incoming variables up to global self.q
                ...
                # S_zz is a square (q x q) matrix, so we pad both the row and column dimensions
                new_S_zz = torch.nn.functional.pad(new_S_zz, (0, pad_size, 0, pad_size))
                
            elif q_new > self.q:
                pad_size = q_new - self.q
                # Pad the existing global variables up to q_new
