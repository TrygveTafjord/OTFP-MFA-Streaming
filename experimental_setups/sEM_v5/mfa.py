...
class MFA(nn.Module):
    ...
    def compute_reconstruction_error(self, X):
        """
        Projects pixels down to the q-dimensional latent space and back up to D-dimensions.
        Returns the Mean Squared Error (MSE) of the reconstruction for each component.
        """
        N, D = X.shape
        errors = []
        ...
        for k in range(self.K):
            ...
            # 1. Math setup (same efficient Woodbury identity as before)
            L_k_scaled = inv_psi.unsqueeze(1) * L_k  # (D, q)
            M = torch.eye(self.q, device=self.device) + L_k.T @ L_k_scaled # (q, q)
            inv_M = torch.inverse(M)                 # (q, q)
            ...
            # 2. Project pixel DOWN to the latent factor space (z)
            proj = diff @ L_k_scaled                 # (N, q)
            z = proj @ inv_M                         # (N, q)
            
            # 3. Project BACK UP to the physical channel space
            reconstruction = z @ L_k.T               # (N, D) (centered)
            
            # 4. Calculate Mean Squared Error (MSE) between original and reconstruction
            # We use MSE so the error scale is independent of the number of channels (D)
            mse = torch.mean((diff - reconstruction)**2, dim=1) # (N,)
            errors.append(mse)
